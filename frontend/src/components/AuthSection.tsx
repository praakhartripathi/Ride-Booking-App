...
export function AuthSection({
  ...
  const [signupForm, setSignupForm] = useState<SignupPayload>({
    ...
    phoneNumber: '',
    password: '',
    role: forceRole ?? 'RIDER',
  })

  useEffect(() => {
    if (forceRole) {
      setSignupForm((current) => ({ ...current, role: forceRole }))
    }
  }, [forceRole])
  ...
  async function handleLogin(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result =
        forceRole === 'DRIVER'
          ? await loginCaptain(loginForm)
          : await loginUser(loginForm)

      login(result)
      setMessage(`Welcome back to QRide, ${result.email}.`)
      navigate(result.role === 'DRIVER' ? '/captain' : '/ride')
    } catch (requestError) {
  ...
  async function handleSignup(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const payload = {
        ...signupForm,
        role: forceRole ?? signupForm.role,
      }
      const result =
        (forceRole ?? signupForm.role) === 'DRIVER'
          ? await registerCaptain(payload)
        ...
        <div className="space-y-8">
          <div className="space-y-5">
            ...
            <p className="max-w-xl text-lg leading-8 text-slate-600">
              Book rides, drive on your schedule, and manage your account from one
              QRide experience. Login and signup are connected directly to the
              Spring Boot backend.
            </p>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <article className="rounded-[28px] border border-black/10 bg-[#f3f3ee] px-5 py-6">
              ...
              <p className="mt-2 text-sm text-slate-600">Connected to auth endpoints</p>
            </article>
        ...
        <div className="self-start rounded-[32px] border border-black/10 bg-white p-6 shadow-[0_30px_90px_-45px_rgba(0,0,0,0.45)] md:p-8">
          ...
          <div className="mt-7 space-y-2">
            ...
            <p className="text-sm leading-6 text-slate-500">
              {mode === 'login'
                ...
                : forceRole === 'DRIVER'
                  ? 'Create a driver account and start accepting rides.'
            ...
            <form className="mt-7 space-y-4" onSubmit={handleSignup}>
              ...
              <div className="grid gap-4 sm:grid-cols-[1fr_180px]">
                ...
                <label className="block space-y-2">
                  ...
                  <select
                    className="w-full rounded-2xl border border-black/10 bg-[#f8f8f4] px-4 py-4 outline-none transition focus:border-black"
                    value={forceRole ?? signupForm.role}
                    disabled={Boolean(forceRole)}
                    onChange={(event) =>
                      setSignupForm((current) => ({
                        ...current,
                        role: event.target.value as SignupPayload['role'],
                      }))
                    }
                  >
          ...
          <div className="mt-6 rounded-[28px] bg-[#f3f3ee] p-5">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">
              Requests are sent to <code className="rounded bg-white px-2 py-1">{API_BASE_URL}</code>
            </p>
            <p className="mt-2 text-sm leading-6 text-slate-600">
              Auth routes use <code className="rounded bg-white px-2 py-1">/api/auth/*</code> and
              captain routes use <code className="ml-1 rounded bg-white px-2 py-1">/api/captain/*</code>.
            </p>
          </div>
        </div>
      </div>
